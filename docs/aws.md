...
```bash
podman run -it --rm quay.io/redhat-developer/mapt:0.6.7 aws host create -h
create
...
podman run -d --name mapt-rhel \
        ...
        quay.io/redhat-developer/mapt:0.6.7 aws host create \
...
podman run -d --name mapt-rhel \
        ...
        -e AWS_SECRET_ACCESS_KEY=XXX \
        -e AWS_DEFAULT_REGION=us-east-1 \
        quay.io/redhat-developer/mapt:0.6.7 aws host destroy \
            --project-name mapt-rhel \
            --backed-url file:///workspace 
```
