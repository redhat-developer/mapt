// Copyright 2026, Pulumi Corporation.  All rights reserved.

// THIS IS AN AUTOGENERATED FILE. DO NOT EDIT THIS FILE DIRECTLY.
// RUN 'make openapi_all' TO REFRESH.

package apitype

// TeamEnvironmentSettings contains additional settings a team applies to an environment.
type TeamEnvironmentSettings struct {
	// The project containing the environment.
	ProjectName string `json:"projectName" yaml:"projectName"`
	// The environment within the project.
	EnvName string `json:"envName" yaml:"envName"`
	// The permission level the team has on this environment.
	// On editEnvironmentPermission, set to "none" to remove the environment from the team. addEnvironmentPermission ignores "none" and stores
	// nothing: adding is how a grant is created, and removeEnvironment is how one is revoked.
	Permission EnvironmentPermission `json:"permission" yaml:"permission"`
	// The maximum duration an environment session can remain open, as a Go duration string (e.g. "1h30m").
	MaxOpenDuration *string `json:"maxOpenDuration,omitempty" yaml:"maxOpenDuration,omitempty"`
	// Display name of the permission set for this environment, when available. Enables read-only entity access UI without requiring RoleRead.
	PermissionSetName *string `json:"permissionSetName,omitempty" yaml:"permissionSetName,omitempty"`
}
