// Copyright 2026, Pulumi Corporation.  All rights reserved.

// THIS IS AN AUTOGENERATED FILE. DO NOT EDIT THIS FILE DIRECTLY.
// RUN 'make openapi_all' TO REFRESH.

package apitype

import (
	"encoding/json"
)

// ResourceResult is the user-facing type for our indexed resources.
// If you add a property here, don't forget to update fieldMappings to make it
// queryable!
type ResourceResult struct {
	// The ISO 8601 timestamp when the resource was first indexed.
	Created *string `json:"created,omitempty" yaml:"created,omitempty"`
	// Whether this is a custom resource managed by a provider plugin.
	Custom *bool `json:"custom,omitzero" yaml:"custom,omitempty"`
	// Whether this resource is pending deletion.
	Delete *bool `json:"delete,omitzero" yaml:"delete,omitempty"`
	// URNs of resources that this resource depends on.
	Dependencies []string `json:"dependencies,omitempty" yaml:"dependencies,omitempty"`
	// Whether the lifecycle of this resource is not managed by Pulumi.
	External *bool `json:"external,omitzero" yaml:"external,omitempty"`
	// The provider-assigned resource ID.
	ID *string `json:"id,omitempty" yaml:"id,omitempty"`
	// Matched search terms mapped to their highlighted values.
	Matches map[string][]json.RawMessage `json:"matches,omitempty" yaml:"matches,omitempty"`
	// The ISO 8601 timestamp when the resource was last updated in the index.
	Modified *string `json:"modified,omitempty" yaml:"modified,omitempty"`
	// The module that contains this resource.
	Module *string `json:"module" yaml:"module"`
	// The name of the resource.
	Name *string `json:"name,omitempty" yaml:"name,omitempty"`
	// The package that provides this resource.
	Package *string `json:"package" yaml:"package"`
	// The URN of the parent resource, if any.
	Parent *string `json:"parent_urn,omitempty" yaml:"parent_urn,omitempty"`
	// The pending operation on this resource, if any (e.g. creating, updating, deleting).
	Pending *string `json:"pending,omitempty" yaml:"pending,omitempty"`
	// The project that contains this resource.
	Program *string `json:"project,omitempty" yaml:"project,omitempty"`
	// Whether this resource is protected from deletion.
	Protected *bool `json:"protected,omitzero" yaml:"protected,omitempty"`
	// The URN of the provider for this resource.
	ProviderURN *string `json:"provider_urn,omitempty" yaml:"provider_urn,omitempty"`
	// The stack that contains this resource.
	Stack *string `json:"stack,omitempty" yaml:"stack,omitempty"`
	// The full type token of the resource (e.g. aws:s3/bucket:Bucket).
	Type *string `json:"type,omitempty" yaml:"type,omitempty"`
	// The URN uniquely identifying this resource within a stack.
	URN *string `json:"urn,omitempty" yaml:"urn,omitempty"`
	// The teams that have access to this resource.
	Teams teamNameSlice `json:"teams,omitempty" yaml:"teams,omitempty"`
	// The resource's input/output properties as a JSON object. Only populated when explicitly requested.
	Properties *RawProperty `json:"properties,omitempty" yaml:"properties,omitempty"`
	// Additional metadata associated with the resource.
	Metadata json.RawMessage `json:"metadata,omitempty" yaml:"metadata,omitempty"`
	// The category of the resource.
	Category *string `json:"category,omitempty" yaml:"category,omitempty"`
	// The Insights account name that discovered or manages this resource.
	Account *string `json:"account,omitempty" yaml:"account,omitempty"`
	// URNs of resources that depend on this resource.
	Dependents []string `json:"dependents,omitempty" yaml:"dependents,omitempty"`
	// What manages this resource's lifecycle: 'Pulumi' for a resource in a Pulumi IaC stack, 'Terraform' / 'CloudFormation' / 'ARM' for one
	// discovered under another IaC tool, and 'Other' for one Insights scanning found under no known tool.
	Managed string `json:"managed,omitempty" yaml:"managed,omitempty"`
	// A fingerprint uniquely identifying this resource's state.
	Fingerprint *string `json:"fingerprint,omitempty" yaml:"fingerprint,omitempty"`
	// The number of sources for this resource.
	SourceCount *int `json:"sourceCount,omitzero" yaml:"sourceCount,omitempty"`
}
