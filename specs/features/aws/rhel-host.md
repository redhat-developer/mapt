...
## Requirements
- [ ] Provision a RHEL EC2 instance (versions: 9.x, 8.x) for x86_64 or arm64
- [ ] Register with Red Hat Subscription Manager using `SubsUsername` / `SubsPassword` via cloud-init
- [ ] Support spot instance allocation with cross-region best-bid selection
- [ ] Support on-demand allocation using the default AWS region
- [ ] Support airgap topology: two-phase stack update (connectivity ON then OFF)
- [ ] Optionally apply the `profileSNC` cloud-config variant to pre-install SNC dependencies
- [ ] Optionally schedule serverless self-destruct after a given timeout (requires remote BackedURL)
- [ ] Write output files: `host`, `username`, `id_rsa` (and bastion files when airgap)
- [ ] `destroy` cleans up main stack, spot stack (if exists), and S3 state
