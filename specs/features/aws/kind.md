...
## Requirements
- [x] Provision an EC2 instance and install Kind + Docker via cloud-init
- [x] Create a Kind cluster during cloud-init; export kubeconfig
- [x] Support configurable Kubernetes version (via Kind node image)
- [x] Support spot instance allocation
- [x] Write output files: `host`, `username`, `id_rsa`, `kubeconfig`
- [x] `destroy` cleans up stack and S3 state
