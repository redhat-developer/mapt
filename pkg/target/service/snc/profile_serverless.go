...
func deployServerlessOperator(ctx *pulumi.Context, args *ProfileDeployArgs) (pulumi.StringOutput, error) {
	goCtx := ctx.Context()
...
func deployKnativeServing(ctx *pulumi.Context, args *ProfileDeployArgs, operatorReady pulumi.StringOutput) (pulumi.Resource, pulumi.StringOutput, error) {
	goCtx := ctx.Context()
	rn := func(suffix string) string {
		return fmt.Sprintf("%s-serverless-%s", args.Prefix, suffix)
	}
	...
	return ks, ksReady, nil
}

// deployKnativeEventing creates a KnativeEventing CR and waits for it to be ready.
// The operatorReady output is used to chain the dependency on the operator installation.
func deployKnativeEventing(ctx *pulumi.Context, args *ProfileDeployArgs, operatorReady pulumi.StringOutput) (pulumi.Resource, error) {
	goCtx := ctx.Context()
	rn := func(suffix string) string {
		return fmt.Sprintf("%s-serverless-%s", args.Prefix, suffix)
	}
