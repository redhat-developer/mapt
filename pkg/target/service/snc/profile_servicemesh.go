...
// deployServiceMesh installs OpenShift Service Mesh 3 and returns the last
// resource together with a StringOutput that resolves when Istio is fully ready.
func deployServiceMesh(ctx *pulumi.Context, args *ProfileDeployArgs) (pulumi.Resource, pulumi.StringOutput, error) {
	goCtx := ctx.Context()
