...
func (r *openshiftSNCRequest) deploy(ctx *pulumi.Context) error {
	...
	// Userdata
	udB64, udDependecies, err := r.userData(ctx, &keyResources.PrivateKey.PublicKeyOpenssh, &lbEIP.PublicIp)
	if err != nil {
		return err
	}
	// Create instance
...
) (pulumi.StringPtrInput, []pulumi.Resource, error) {
	// Resources
...
func kubeconfig(ctx *pulumi.Context,
	...
	ocpReadyCmd, err := c.RunCommand(ctx, commandReadiness, *prefix, awsOCPSNCID,
	...
	getKC, err := c.RunCommand(ctx, getKCCmd,
		*prefix, awsOCPSNCID, mk, amiUserDefault,
