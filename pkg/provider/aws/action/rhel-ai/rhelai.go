...
func Create(ctx *maptContext.ContextArgs, r *Request) error {
	...
	vmTypes, err := instanceSpecs.GetMachineTypes()
	if err != nil {
	...
	vmTypes, err = data.FilterInstaceTypesOfferedByRegion(vmTypes, r.region)
	if err != nil {
		return err
	}
	r.vmTypes = vmTypes
...
func (r *Request) deploy(ctx *pulumi.Context) error {
	...
	cr := compute.ComputeRequest{
		...
		InstaceTypes:   r.vmTypes,
		DiskSize:       &diskSize,
		LB:             lb,
		LBTargetGroups: []int{22},
		SpotPrice:      strconv.FormatFloat(r.spotPrice, 'f', -1, 64),
		Spot:           true}
